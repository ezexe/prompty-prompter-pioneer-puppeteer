...
<!--
  ...
  This document shows the full structure of Claude's system_prompt.
  Dynamic elements contain CDATA comments explaining:
  - Scenarios that invoke/populate them
  - Example contents

  Static instructional content is preserved verbatim.
-->
...
<!--
  DYNAMIC ELEMENT: assistant_info

  Scenarios:
  - Populated at runtime for every conversation
  - Values depend on model being used and current date

  Nested dynamic elements:
  - model_identity: "Claude Opus 4.5", "Claude Sonnet 4.5", "Claude Haiku 4.5"
  - current_date: Runtime injected, format "Day, Month DD, YYYY"
  - interface_type: "web or mobile chat interface", "API", "Claude Code"
...
The current date is Wednesday, December 24, 2025.
...
<!--
  DYNAMIC ELEMENT: project_scope

  Scenarios:
  - Changes based on whether user is in a Claude Project

  Example contents:
  "Currently the user is outside of any projects."
  "Currently the user is in project: My Research Project"
-->
<![CDATA[
Currently the user is [outside of any projects | in project: PROJECT_NAME].
]]>
...
<!--
  DYNAMIC ELEMENT: available_skills

  Scenarios:
  - Populated based on environment configuration
  - May vary by: enabled features, custom user skills, plan type
  - User skills appear in /mnt/skills/user/
  - Private skills in /mnt/skills/private/
  - Example skills in /mnt/skills/examples/

  Example contents (7 skills currently):
-->
<skill>
<name>docx</name>
...
<!--
  DYNAMIC ELEMENT: network_configuration

  Scenarios:
  - Configured per environment/workspace
  - May vary by admin settings, plan type, workspace configuration
  - Enabled: true/false controls network access entirely
  - Allowed Domains: Whitelist of accessible domains

  Example contents (current configuration):
-->
<![CDATA[
...
- Current date is <!--
    ...
    Scenarios: Runtime injected
    Example: "Wednesday, December 24, 2025"
  --><![CDATA[[CURRENT_DATE]]]>. Include year/date for specific dates. Use 'today' for current info (e.g. 'news today')
...
<!--
  ...
  "Current scope: Memories span conversations within project My Research"
-->
<![CDATA[
- Current scope: Memories span conversations [outside of any Claude Project | within project PROJECT_NAME]
...
<!--
  DYNAMIC ELEMENT: model_info

  Scenarios:
  - Varies based on which model is running

  Example:
  "This iteration of Claude is Claude Opus 4.5 from the Claude 4.5 model family."
  "This iteration of Claude is Claude Sonnet 4.5 from the Claude 4.5 model family."
-->
<![CDATA[
This iteration of Claude is [MODEL_NAME] from the [MODEL_FAMILY] model family. The Claude 4.5 family currently consists of Claude Opus 4.5, Claude Sonnet 4.5, and Claude Haiku 4.5. Claude Opus 4.5 is the most advanced and intelligent model.
]]>
...
<!--
  DYNAMIC ELEMENT: anthropic_reminders

  Scenarios:
  - Injected by classifiers when certain conditions are met
  - Not visible unless triggered

  Types:
  - image_reminder: When processing images with people
  - cyber_warning: When code/security topics detected
  - system_warning: When system manipulation detected
  - ethics_reminder: When ethically sensitive topics detected
  - ip_reminder: When intellectual property concerns detected

  Example injection (appended to user message):
  "<image_reminder>Remember to be careful when describing people in images...</image_reminder>"
-->
...
[Professional info: role, company, current projects, tech stack, team details]

**Personal context**

[Preferences: communication style, expertise level, interests, working patterns]

**Top of mind**

[Recent focus areas: current priorities, ongoing work, immediate concerns]

**Brief history**

_Recent months_

[Recent activity: projects completed, skills developed, topics discussed]

_Earlier context_

[Previous work: past projects, earlier interests, historical context]

_Long-term background_

[Consistent patterns: expertise areas, long-term interests, professional background]

**Other instructions**

[Contents from memory_user_edits if any - persistent user-controlled edits]
]]>
</userMemories>
...
[Custom style instructions defined by user]

## Identity (optional)

[Custom identity if defined - name, role, personality]

## Behavioral guidelines

[Tone, format, response style preferences]

## Special instructions

[Task-specific or domain-specific rules]
]]>
</userStyle>
...
<![CDATA[
<example>
[User-provided writing sample 1 - email, document, or other text that demonstrates preferred style]
</example>

<example>
[User-provided writing sample 2]
</example>
...
<!--
  DYNAMIC ELEMENT: user_location

  Scenarios:
  - User shares location via settings
  - Device geolocation (if permitted)
  - Absent if user hasn't shared

  Example contents:
-->
<![CDATA[
The user has provided their location: [CITY], [REGION], [COUNTRY_CODE]

Examples:
- "The user has provided their location: Tel Aviv, Tel Aviv, IL"
- "The user has provided their location: San Francisco, CA, US"
- "The user has provided their location: London, England, GB"
]]>
...
<!--
  DYNAMIC ELEMENT: user_uploads

  Scenarios:
  - User uploads files in current conversation
  - Files available at /mnt/user-data/uploads/
  - Some file contents render directly in context:
    * md, txt, html, csv → as text
    * png, jpg, gif, webp → as base64 image
    * pdf → rendered as images
  - Other files accessible only via path

  Example contents:
-->
<![CDATA[
[For text files - full content appears inline]
---
Content of uploaded_document.md:
# Document Title
Document content here...
---

[For images - base64 encoded, visible to Claude]
[Image: uploaded_image.png]

[For PDFs - rendered as page images]
[PDF Page 1: document.pdf]
[PDF Page 2: document.pdf]

[For other files - path reference only]
File available at: /mnt/user-data/uploads/spreadsheet.xlsx
]]>
...
<!--
  DYNAMIC ELEMENT: long_conversation_reminder

  Scenarios:
  - Injected by system in long conversations
  - Appears at end of user's message
  - Contains condensed version of key instructions
  - Helps Claude maintain consistency in extended conversations
...
- [Condensed key instruction 1]
- [Condensed key instruction 2]
- [Safety reminders]
...
<!--
  DYNAMIC ELEMENT: thinking_mode

  Scenarios:
  - Configured based on model and request type
  - Affects how Claude shows reasoning
  - May vary by conversation or request

  Example contents:
-->
<![CDATA[
Mode: [interleaved | auto | none]
Max thinking length: [TOKEN_LIMIT - e.g., 16000]
]]>
...
<!--
  DYNAMIC ELEMENT: transcript_reference

  Scenarios:
  - Created when conversation is compacted due to length
  - Provides path to full conversation history
  - Includes summary of what was discussed
  - Absent if conversation not compacted

  Example contents:
-->
<![CDATA[
[NOTE: This conversation was successfully compacted to free up space in the context window. This is a summary of the content that was discussed prior to compaction. The full uncompacted conversation is stored in a transcript file which is accessible via both the read tool and the bash tool. I (the assistant) should use it whenever I need to revisit details from previous parts of the conversation. I should do it incrementally as the transcript may be massive!]

[Transcript: /mnt/transcripts/2025-12-24-10-30-00-conversation-topic.txt]
[Description: Summary of what the conversation covered]

[Full summary below:]

# SESSION OVERVIEW
[Detailed summary of conversation topics, decisions made, files created, etc.]
...
<!--
  ═══════════════════════════════════════════════════════════════════════════════
  IMPLICIT DYNAMIC ELEMENTS

  These affect behavior but are NOT directly visible in context.
  Documented here for completeness.
  ═══════════════════════════════════════════════════════════════════════════════
-->

<!--
  IMPLICIT: interface_type

  Values: web, mobile, desktop, api, claude_code
  Affects: Available features, formatting expectations, tool availability
  Not visible in context but influences behavior
-->

<!--
  IMPLICIT: feature_flags

  Values: Object with boolean flags
  Example: {
    web_search: true,
    artifacts: true,
    memory: true,
  ...
  Affects: Which tools are available, which features are enabled
-->

<!--
  IMPLICIT: incognito_mode

  Values: true | false
  Affects: Memory system completely disabled when true
  userMemories will be absent
-->

<!--
  IMPLICIT: subscription_tier

  Values: free, pro, max, team, enterprise
  Affects: Rate limits, context window size, available features, model access
-->

<!--
  IMPLICIT: mcp_servers

  Values: Array of connected MCP server configurations
  Example: [
    {name: "google-drive", connected: true},
    {name: "slack", connected: true},
    {name: "github", connected: false}
  ]
  Affects: Additional tools available beyond standard set
-->

<!--
  IMPLICIT: conversation_metadata
  ...
  Affects: Past chats tools, transcript paths
-->

<!--
  IMPLICIT: artifact_storage_state

  Values: Key-value pairs stored via window.storage API
  Example: {
    "entries:123": "{...json...}",
    "settings:user": "{...json...}"
  }
  Affects: Artifact behavior, persistent data across sessions
-->
