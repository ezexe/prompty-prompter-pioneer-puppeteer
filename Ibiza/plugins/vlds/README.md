...
- A reply that ends with acts not done — held on the per-act word, blocked, or optional — closes with the popup, never prose that waits for a typed word: the visualize connector's elicitation form, one option per pending act with a summary and a textbox, all-the-above and other, one submit and re-served at every closing; the native question panel serves the closing only where that tool is absent, and is otherwise the mid-turn fork's. The selection is the word for exactly the selected acts; no VLDS operation is ever an option.
