...
`store/phi-index.md` and `store/arc/` are the gc's φ-register — normalized in-session per its reference, never by hook; the one hook-side arc write is the whole-file dispatch pour above. The index is derived, but a user's edit to it is a ruling — its `## recall` section is where the injected slice is chosen.
