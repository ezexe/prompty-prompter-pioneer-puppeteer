...
The record is this session's dispatch file — the one named by `.dispatch-current` in the store — session-scoped and never promoted to a rule; where echoes come from is in [reference.md](reference.md).
