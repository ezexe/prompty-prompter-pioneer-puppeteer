...
The plugin's hooks (`hooks/vlds_hooks.py`) are its mechanical peers: the SessionStart hook injects the recall, the prompt hook stamps dispatch rows (naming the session by its short id and its chat title) and pours `dispatch.md` whole-file at a new session's first prompt, the copy named after the session whose rows it holds, the pre-write hook asks before a store-named file is written outside a `.claude/vlds/` directory or a persisted entry carries a placeholder `time:` or one later than the latest `now:` by more than a minute, and the post-write hook runs `check` after any store write or store-named write; every hook output carries the clock (`now:`) the model copies its stamps from.
...
**The normalize sweep** (`/vlds:gc normalize <file | 'register'>`) — gc-owned: the judgment is the collector's, and it runs whenever the check shows the work owed.
