...
`.vlds/logger.md` is the looper's own — the dashboard's single, append-only, user-editable activity log. Every instrument's decision is logged there as the looper runs, tagged by instrument:
