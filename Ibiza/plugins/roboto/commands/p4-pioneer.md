...
1. **Closure** — every selected item's `metadata.p4.depends_on` is satisfied by the selection.
   Note the two id-spaces: skill names resolve to `skills/<name>/`, while P4 **layer** ids
   (`prompty | prompter | pioneer | puppeteer`) resolve to `docs/fragments.md` sections — do not
