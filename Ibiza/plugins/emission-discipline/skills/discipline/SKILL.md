...
| R22  | a fence, a copyable block, or the answer itself is about to be emitted before a tool call the turn will still make — the record script's run, an operator hand-off, the closing popup | hold it for the final message: the text after the turn's last tool call is the delivery surface, since the Code tab drops fences and widgets placed before or beside the closing tool calls while prose there is seen; the bookkeeping runs first, the popup is the last tool call, and the final message carries the deliverable, the bookkeeping line, and the question the popup asks; the derivation line and the plan fence before the acts are process text and carry no deliverable |
...
### Rule 22 — the delivery surface

22. **The final message is the delivery surface.** Fires when a fence, a copyable block, or the answer itself is about to be emitted before a tool call the turn will still make — the record script's run, an operator's hand-off, the closing popup. The Claude desktop app's Code tab does not reliably show fenced blocks or widgets placed before or beside a turn's closing tool calls, while prose placed there is seen: an owner answered "what picker?" to a widget served after the reply text, and "theres no laid out fence" to a fence that sat before the record script's call. So the bookkeeping runs first, the popup is the turn's last tool call, and everything the asker must read — every fence, the answer, the one-line bookkeeping report, and the question the popup asks — goes in the final message after that last call, where a lost widget still leaves a readable question. The derivation line and the plan fence that precede the acts are process text: they may not carry the deliverable. Acceptance: an owner reading only the turn's final message sees the deliverable, the question, and the bookkeeping line without scrolling into tool output.
